...
$header_class = array( 'site-header', 'site-header-style-5' );
		...
		<div class="container">
			...
			<div class="header-bottom-banner">
				<div class="header-banner">
					<?php 
					$banner_img = tisara_theme_mod('tisara_header_banner_image');
					if ( empty( $banner_img ) ) {
						$banner_img = get_template_directory_uri().'/assets/img/banner.jpg';
					}
					$banner_url = tisara_theme_mod('tisara_header_banner_url');
					if ( empty( $banner_url ) ) {
						$banner_url = '#';
					}
					?>
					<a href="<?php echo esc_url( $banner_url ); ?>">
						<img src="<?php echo esc_url( $banner_img ); ?>" alt="<?php esc_attr_e( 'Header Banner', 'tisara-wp' ); ?>">
					</a>
				</div>
			</div>
		</div>
